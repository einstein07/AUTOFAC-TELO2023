...
boost::shared_ptr<RobogenConfig> ConfigurationReader::parseConfigurationFile(
	...
	desc.add_options()
			...
			("resourcesConfigFile", boost::program_options::value<std::string>(),
			...
			("gatheringZonePosition",
					boost::program_options::value<std::string>(),
					"The position of the gathering zone: x,y,z (comma separated)."\
					"The specified position will be the position of the center of the "\
					"gathering zone.\n")
			("gatheringZoneSize",
					boost::program_options::value<std::string>(),
	...
	// SM added - Read resources configuration
	...
	if (!vm.count("resourcesConfigFile")) {
		...
		makeAbsolute(resourcesConfigFile, filePath);

		resources = parseResourcesFile(resourcesConfigFile);
		if (resources == NULL) {
				return boost::shared_ptr<RobogenConfig>();
		}
	}
	...
	if(vm.count("gatheringZoneSize")) {
		...
		std::string zoneString = vm["gatheringZoneSize"].as<std::string>();
		std::vector<std::string> zoneOpts;
		boost::split(zoneOpts, zoneString, boost::is_any_of(","));
		if (zoneOpts.size() == 3) {
			for(unsigned int i=0; i<3; ++i) {
				zoneSize[i] = std::atof(zoneOpts[i].c_str());
			}
		} else {
			std::cerr << "'gatheringZoneSize' must be "
					"x-length,y-length,z-length (comma separated)."
					<< std::endl;
			return boost::shared_ptr<RobogenConfig>();
		}
	}
	...
	return boost::shared_ptr<RobogenConfig>(
			new RobogenConfig(scenario, scenarioFile, nTimeSteps,
					...
					startPositionFile, resources, resourcesConfigFile,
					lightSources, lightSourcesFile, sensorNoiseLevel,
...
boost::shared_ptr<ResourcesConfig> ConfigurationReader::parseResourcesFile(
	...
	if (!resourcesFile.is_open()) {
		std::cout << "Cannot find resources file: '" << fileName << "'"
				<< std::endl;
...
boost::shared_ptr<RobogenConfig> ConfigurationReader::parseRobogenMessage(
	...
	for (int i = 0; i < simulatorConf.resources_size(); ++i) {
		...
		const robogenMessage::Resource& r = simulatorConf.resources(i);
	...
	boost::shared_ptr<GatheringZoneConfig> gatheringZone;
        osg::Vec3 position = osg::Vec3(
                                        simulatorConf.gatheringzonex(),
                                        simulatorConf.gatheringzoney(),
										simulatorConf.gatheringzonez()
                                    );
