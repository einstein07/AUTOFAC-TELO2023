...
float RobotRepresentation::calculateNoveltyScore(const std::vector<boost::shared_ptr<RobotRepresentation> > & noveltyArchive , const std::vector<boost::shared_ptr<RobotRepresentation> > & population ){
        std::cout<<"CALC NOVELTY SCORE. . ."<<std::endl;
	...
	for(auto r: noveltyArchive){
        ...
        std::cout<<"PUSHING BACK DISTANCES OF INDIVIDUALS IN NOVELTY ARCHIVE."<<std::endl;
	for(auto r: population){
		distances.push_back(this->euclideanDistance(r->getEndPosition()));
	}
        std::cout<<"PUSHING BACK DISTANCES OF INDIVIDUALS IN POPULATION."<<std::endl;
        
	std::sort(distances.begin(),distances.end());
        std::cout<<"SORTED DISTANCES - "<<distances.size()<<std::endl;
	float avgDistance=0;
	for(int i=1; i<16; i++){
		avgDistance += distances[i];
	}
	avgDistance = avgDistance/15;
        std::cout<<"NOVELTY SCORE FOR CURRENT INDIVIDUAL: "<<avgDistance<<std::endl;
	//this->setNoveltyScore(avgDistance);
        this->noveltyScore=avgDistance;
        std::cout<<"SET NOVELTY SCORE FOR CURRENT INDIVIDUAL."<<std::endl;
