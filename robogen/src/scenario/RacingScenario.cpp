...
bool RacingScenario::endSimulation() {
	...
	for (unsigned int i = 0; i < bodyParts.size(); ++i) {
		osg::Vec3 curBodyPos = osg::Vec3(bodyParts[i]->getRootPosition().x(), bodyParts[i]->getRootPosition().y(), bodyParts[i]->getRootPosition().z());
		osg::Vec3 curDistance = startPosition_[startPosition_.size()-1] - curBodyPos;
