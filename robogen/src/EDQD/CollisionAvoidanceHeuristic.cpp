...
namespace robogen{
	...
	osg::Vec2d CollisionAvoidanceHeuristic::step(){
		...
		if (robot_ -> isBoundToResource()){
			...
			boost::shared_ptr<BoxResource> resource = scenario_ -> getEnvironment()->getResources()[rId];
		...
		for( unsigned int i = 0 ; i < robot_->getSensors().size(); ++i ){
			...
			//std::cout << "Sensor position "<< i << " is type " << robot_->getSensors()[i]->getLabel() << std::endl;
			if (boost::dynamic_pointer_cast<TargetAreaDetectorElement>(robot_->getSensors()[i])) {

				if ( boost::dynamic_pointer_cast<TargetAreaDetectorElement>(robot_->getSensors()[i])->
										read()){
						//std::cout << "Within target area" << std::endl;
						if ( !robot_->isBoundToResource() ){
			...
			else if (boost::dynamic_pointer_cast< IrSensorElement>(robot_->getSensors()[i])) {
				...
				if ( boost::dynamic_pointer_cast< IrSensorElement>(robot_->getSensors()[i])->
					...
					if (boost::dynamic_pointer_cast< SensorElement>(robot_->getSensors()[i + SensorElement::WALL])) {
						if ( boost::dynamic_pointer_cast< SensorElement>(robot_->getSensors()[i + SensorElement::WALL])->
							...
							int targetIndex = boost::dynamic_pointer_cast<SensorElement>(robot_->getSensors()[i + SensorElement::WALL])
																											->getObjectId();
							pos = scenario_-> getEnvironment() -> getObstacles()[targetIndex]-> getPosition();
							//if (distance (pos, robot_->getCoreComponent()->getRootPosition()) < minDistance){
								//osg::Vec3d localPos = getLocalPoint(pos);
								pos = osg::Vec3d(0, 0, 0);
								//std::cout << "COLLISION POSSIBLE WITH WALL - will drive to: (" << pos.x() << ", " << pos.y() << ")." << std::endl;
					...
					if (d < minDistance){
						...
						if (boost::dynamic_pointer_cast< SensorElement>(robot_->getSensors()[i + SensorElement::ROBOT])) {
							if ( boost::dynamic_pointer_cast< SensorElement>(robot_->getSensors()[i + SensorElement::ROBOT])->
								...
								int targetIndex = boost::dynamic_pointer_cast<SensorElement>(robot_->getSensors()[i + SensorElement::ROBOT])
																				->getObjectId();
