...
public:
	...
	/**
	 * SM added - visualize swarm
	 * @return true if scene was configured correctly, false otherwise
	 */
	bool configureScene(std::vector<std::vector<boost::shared_ptr<Model> > > swarmBodyParts,
			boost::shared_ptr<Scenario> scenario);
	bool done();
...
private:
	...
	//SM added
	std::vector<std::vector<boost::shared_ptr<RenderModel> > > swarmRenderModels_;
