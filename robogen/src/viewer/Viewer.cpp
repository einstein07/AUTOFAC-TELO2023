...
bool Viewer::configureScene(std::vector<std::vector<boost::shared_ptr<Model> > > swarmBodyParts,
                        boost::shared_ptr<Scenario> scenario){
	swarmRenderModels_.resize(swarmBodyParts.size());
    for (unsigned int i = 0; i < swarmBodyParts.size(); ++i){
        for (unsigned int j = 0; j < swarmBodyParts[i].size(); ++j) {
            boost::shared_ptr<RenderModel> renderModel =
                            RobogenUtils::createRenderModel(swarmBodyParts[i][j]);
            if (!renderModel) {
				std::cout
				<< "Cannot create a render model for model "
				<< j << " robot " << i << std::endl;
            ...
            swarmRenderModels_[i].push_back(renderModel);
            this->root->addChild(swarmRenderModels_[i][j]->getRootNode());
        }
    }
	...
	// Gathering zone render model
	boost::shared_ptr<GatheringZone> gatheringzone =
			scenario->getEnvironment()->getGatheringZone();
	if (gatheringzone){
